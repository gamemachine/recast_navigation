...
int NavigationMesh::Init(float cellTileSize, int tileBits, int polyBits)
{
	// Allocate objects
...
dtNavMesh* NavigationMesh::GetDtNavMesh()
{
	return m_navMesh;
}
...
void NavigationMesh::FindPath(NavMeshPathfindQuery query, NavMeshPathfindResult* result)
{
	...
	vector<float3> straightPath;
	vector<uint8_t> straightPathFlags;
	vector<dtPolyRef> straightpathPolys;
...
void NavigationMesh::Raycast(NavMeshRaycastQuery query, NavMeshRaycastResult* result)
{
	...
	status = m_navQuery->raycast(startPoly, &query.start.x, &query.end.x, &filter, &t, &result->normal.x, polys.data(), &raycastPolyCount, query.maxPathPoints);
	if (dtStatusFailed(status))
		return;
